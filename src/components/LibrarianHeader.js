...
export default function LibrarianHeader({ title = 'Dashboard', subtitle, onSearch }) {
  ...
  const handleSearch = (e) => {
    ...
    if (onSearch) {
      ...
      router.push(`/librarian/inventory?search=${encodeURIComponent(searchQuery)}`);
    }
  };
    ...
    <header className="h-20 shrink-0 px-8 flex items-center justify-between border-b border-white/5 bg-background-dark/80 backdrop-blur-xl sticky top-0 z-10">
      <div className="flex flex-col">
        <h2 className="text-xl font-bold text-white tracking-tight flex items-center gap-2">
          {title}
          <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-primary/20 text-primary border border-primary/20">DASHBOARD</span>
        </h2>
        <p className="text-xs text-white/40 font-medium">
          {subtitle || `Welcome back, ${userData?.name?.split(' ')[0] || 'Librarian'}.`}
        </p>
      </div>
